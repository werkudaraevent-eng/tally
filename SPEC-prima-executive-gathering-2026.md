...
- `code` **tidak dapat diubah** setelah penawaran dibuat: dipakai di database dan laporan.
- `scope` dan `booth_id` **masih dapat diubah selama penawaran belum pernah diklaim**. Begitu ada klaim, keduanya terkunci (`OFFER_SCOPE_LOCKED_CLAIMED`) karena klaim tercatat terhadap cakupan yang berlaku saat transaksi terjadi; mengubahnya membuat laporan menyatakan aturan yang tidak pernah ada.
- Cakupan penawaran **bawaan booth tidak dapat diubah sama sekali** (`OFFER_SCOPE_LOCKED_BUILTIN`), ditegakkan trigger `guard_builtin_offer_scope` di database. Alasannya tidak ada jalan pulih: kalau baris builtin sempat menjadi `global`/`booth_id = NULL`, trigger sinkronisasi booth **tidak dapat memperbaikinya** karena klausa `on conflict do update`-nya tidak menyertakan `scope` maupun `booth_id`, sehingga item diskon booth itu muncul di semua booth secara permanen. Flag `is_builtin` sendiri juga dikunci untuk menutup celah dua langkah (lepas flag, lalu ubah cakupan).
