  ...
  return <main className="bg-surface px-5 py-6 text-on-surface sm:px-8 lg:py-10">
    <div className="mx-auto max-w-[1440px]">
      ...
      <div className="mt-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          <p className="text-body-small font-semibold uppercase tracking-[0.2em] text-primary">User & role management</p>
          <h1 className="mt-3 text-4xl font-semibold tracking-[-0.06em] sm:text-5xl">{canManage ? "Kelola panitia." : "Daftar panitia."}</h1>
          <p className="mt-3 text-body-medium leading-6 text-on-surface-variant">{canManage
        ...
        <section className="space-y-6">
          <div className="rounded-lg border border-outline-variant bg-panel p-5">
            ...
            <label className="mt-4 block text-body-medium font-semibold">PIN 6 digit {draft.id && <span className="font-normal text-on-surface-variant">(kosongkan bila tidak diubah)</span>}
              <input value={draft.pin} inputMode="numeric" maxLength={6} onChange={(event) => setDraft((current) => ({ ...current, pin: event.target.value.replace(/\D/g, "").slice(0, 6) }))} placeholder="••••••" className="rounded-md mt-2 h-12 w-full border border-outline-variant bg-surface px-3 text-lg tracking-[0.3em] outline-none focus:border-primary" />
            </label>
