      ...
      {!settings ? <p className="mt-8 text-body-medium text-on-surface-variant">Memuat settings...</p> : <div className="mt-8 space-y-2">
        ...
        <section className="rounded-lg bg-panel p-6">
          ...
          <label className="mt-4 block text-body-medium font-semibold">Auto-void order pending setelah (menit)
            <input type="number" min={5} max={1440} value={settings.pending_auto_void_minutes} onChange={(event) => setSettings((current) => current && { ...current, pending_auto_void_minutes: Math.max(5, Math.min(1440, Number(event.target.value) || 5)) })} className="rounded-md mt-2 h-12 w-32 border border-outline-variant bg-surface px-3 text-lg tabular-nums outline-none focus:border-primary" />
          </label>
          <p className="mt-3 text-body-small text-on-surface-variant">Pengaturan leaderboard (tampil/sembunyi & privasi nama) kini ada di halaman <Link href="/admin/display" className="font-semibold text-primary">Live Display</Link>.</p>
        </section>
