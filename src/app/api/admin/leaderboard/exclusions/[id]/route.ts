...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data: current } = await client.from("leaderboard_exclusions").select(COLUMNS).eq("id", id).maybeSingle();
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await client.from("leaderboard_exclusions").delete().eq("id", id);
  if (error) return apiError(mapDatabaseError(error), 500);
