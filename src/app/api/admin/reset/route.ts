...
export async function POST(request: Request) {
  ...
  // p_event_id WAJIB di RPC. DIUKUR sebelum RPC discope: reset satu event
  // menghapus order event LAIN juga (1 -> 0), tanpa galat dan tanpa jalan pulih.
  const { data, error } = await getSupabaseServiceClient().rpc(
    "admin_reset_records" as never,
    {
      p_event_id: auth.scope.event.id,
      p_actor: auth.user.id,
      p_include_participants: parsed.data.include_participants,
    } as never,
  );
