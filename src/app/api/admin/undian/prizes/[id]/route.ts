...
const patchSchema = z.object({
  ...
  animation: z.enum(["wheel", "slot", "cards", "digits", "instant"]).optional(),
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data: current } = await client.from("undian_prizes").select(PRIZE_COLUMNS).eq("id", id).maybeSingle();
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await client.from("undian_prizes").delete().eq("id", id);
  if (error) return apiError("INTERNAL_ERROR", 500);
