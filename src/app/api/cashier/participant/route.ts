...
export async function GET(request: Request) {
  ...
  const participantQuery = await client.from("participants").select("id,qr_code,name,company,title,photo_url,allow_name_display").eq("qr_code", parsed.data.qr).single();
  ...
  const { data: orders, error: ordersError } = await client.from("orders").select("id,code,participant_id,booth_id,has_discount_item,regular_amount,total_amount,status,pickup_mode,note,created_at,payment_method,approval_code,paid_at,handed_over_at,void_reason").eq("participant_id", participant.id).eq("status", "pending").order("created_at", { ascending: true });
