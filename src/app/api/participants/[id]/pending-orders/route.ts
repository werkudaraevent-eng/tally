...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: participant, error: participantError }, { data: orders, error: ordersError }] = await Promise.all([
    client.from("participants").select("id,qr_code,name,company,title,photo_url,allow_name_display").eq("id", params.data.id).single(),
    client.from("orders").select("id,code,participant_id,booth_id,has_discount_item,regular_amount,total_amount,status,pickup_mode,note,created_at,payment_method,approval_code,paid_at,handed_over_at,void_reason").eq("participant_id", params.data.id).eq("status", "pending").order("created_at", { ascending: true }),
  ]);
