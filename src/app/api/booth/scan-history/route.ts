...
export async function GET(request: Request) {
  ...
  const filtered = auth.scope.role === "booth" ? rows.filter((row) => row.payload?.booth_id === auth.scope.boothId) : rows;
  const participantIds = filtered.map((row) => row.payload?.participant_id).filter((id): id is string => Boolean(id));
  const { data: participants } = participantIds.length ? await getSupabaseServiceClient().from("participants").select("id,name,company").eq("event_id", eventId).in("id", participantIds) : { data: [] as Array<{ id: string; name: string; company: string | null }> };
