...
export default async function EventWorkspace({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const canOperate = event.status === "active";
  ...
  return <main className="min-h-dvh bg-[var(--background)] px-5 py-6 text-[var(--ink)] sm:px-8 lg:py-10"><div className="mx-auto max-w-[1100px]">
    ...
    <header className="mt-5 border-b border-[var(--line)] pb-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><span className="border border-[var(--line)] px-2 py-1 text-xs font-semibold uppercase">{event.status}</span><h1 className="mt-4 text-4xl font-semibold tracking-[-0.05em]">{event.name}</h1><p className="mt-3 text-sm text-[var(--ink-muted)]">Workspace terisolasi · {event.slug}</p></div>{role === "super_admin" || role === "admin" ? <Link href={`/e/${slug}/settings`} className="flex min-h-11 items-center gap-2 border border-[var(--line)] px-4 text-sm font-semibold"><GearSix size={18} /> Konfigurasi event</Link> : null}</div></header>
    {!canOperate && <div className="mt-6 border border-[var(--warning)]/30 bg-[var(--warning)]/5 p-4 text-sm"><strong>Event masih draft.</strong> Siapkan konfigurasi dan user sebelum diaktifkan. Transaksi belum dibuka.</div>}
    <section className="mt-8 grid gap-px border border-[var(--line)] bg-[var(--line)] sm:grid-cols-2 lg:grid-cols-3">
      {(role === "admin" || role === "super_admin") && <Link href={`/e/${slug}/admin`} aria-disabled={!canOperate} className={`bg-[var(--surface)] p-6 ${!canOperate ? "pointer-events-none opacity-45" : "hover:bg-[var(--surface-muted)]"}`}><GearSix size={26} className="text-[var(--brand)]" /><h2 className="mt-8 text-lg font-semibold">Admin</h2><p className="mt-2 text-sm text-[var(--ink-muted)]">Peserta, booth, transaksi, laporan, dan CMS.</p></Link>}
      {(role === "booth" || role === "admin" || role === "super_admin") && <Link href={`/e/${slug}/booth`} aria-disabled={!canOperate} className={`bg-[var(--surface)] p-6 ${!canOperate ? "pointer-events-none opacity-45" : "hover:bg-[var(--surface-muted)]"}`}><Storefront size={26} className="text-[var(--brand)]" /><h2 className="mt-8 text-lg font-semibold">Booth</h2><p className="mt-2 text-sm text-[var(--ink-muted)]">Scan peserta dan catat transaksi.</p></Link>}
