...
import { CheckCircle, MagnifyingGlass, Package, Prohibit, Scan, Storefront, UserCircle, XCircle } from "@phosphor-icons/react";
...
export default function BoothPage() {
  ...
  async function createOrder() {
    ...
    if (!booth) { setMessage("Booth belum termuat. Muat ulang halaman."); return; }
    setPending(true); setMessage("");
    const response = await fetch("/api/orders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ order_code: `${booth.code}-${orderCode.padStart(3, "0")}`, participant_id: participant.id, booth_id: booth.id, has_discount_item: discount, regular_amount: Number(regularAmount) || 0, offer_codes: selectedOffers }) });
    const data = await response.json();
    setPending(false);
    if (!response.ok) {
  ...
  const previewTotal = (Number(regularAmount) || 0) + selectedOfferTotal;
