      ...
      case "CONDITIONS_NOT_MET": {
        ...
        return failed.map((item) => {
          if (item.var === "total_spend") {
            ...
            return `Perlu belanja ${scope} ${formatRupiah(item.value ?? 0)} — baru ${formatRupiah(Number(item.actual ?? 0))}`;
          }
          if (item.var === "booth_count") return `Perlu kunjungi ${item.value} booth — baru ${item.actual} booth`;
          if (item.var === "participant_type") return `Khusus tipe ${(item.values ?? []).join("/")} — peserta ini ${item.actual ?? "-"}`;
